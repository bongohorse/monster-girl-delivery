...
The project retains the **variable-delta simulation policy** established by [#143](https://github.com/bongohorse/monster-girl-delivery/issues/143) / [PR #187](https://github.com/bongohorse/monster-girl-delivery/pull/187), after the local flight and collision corrections in [#176](https://github.com/bongohorse/monster-girl-delivery/issues/176) and [#173](https://github.com/bongohorse/monster-girl-delivery/issues/173). Delta-scaled movement alone does not prove frame-partition independence; collision and discrete transitions need explicit treatment and regression evidence.
...
- **Profiling isolation**: Profiling telemetry and measured game-step wall-clock intervals in the Director performance HUD are presentation and diagnostic tools only; they must never feed back into `TimeService` or influence gameplay simulation. Phaser RAF-level `actualFps` / `rawDelta` are not used as capped-game-step evidence because the limited loop can still update those values on every browser RAF callback.

Continuous collision detects positive-area logical AABB overlap anywhere in the lethal portion of a step, including player boundary contacts and patrol reversals; edge-only contact stays safe. It does not expose time of impact. `stepPrototypeRun` retains the enclosing-step endpoint when entering `dead`, so terminal time, distance, and pose remain schedule-relative even when collision outcomes agree. Dead simulation steps preserve state; scene resize can still constrain flight pose without advancing distance. M5 consumers must use the [#182 run-end contract](docs/PRE_M5_RUN_METRICS_EVENT_AUTHORITY_AUDIT.md) and [#184 consequence handoff](docs/PRE_M5_CONSEQUENCE_OWNERSHIP_AUDIT.md), rather than assume an exact impact metric or an immutable scene result.

Executable partition evidence lives in [FramePartitionSimulation.test.ts](tests/systems/FramePartitionSimulation.test.ts) and [SystemPartitionEvidence.test.ts](tests/systems/SystemPartitionEvidence.test.ts); collision and live wiring regressions are in [HazardCollision.test.ts](tests/systems/HazardCollision.test.ts) and [Foundation.test.ts](tests/game/scenes/Foundation.test.ts). These tests compare equivalent simulation-time/input scenarios; they do not certify manual device feel.
...
The live hazard domain now follows the effective logical flight bounds through `PrototypeHazardVerticalDomain` ([#191 / PR #192](https://github.com/bongohorse/monster-girl-delivery/pull/192)). Generation and reachability share the adapted catalog and validation bounds. Baseline geometry is preserved; added vertical room redistributes future hazard centers and expands reactive target limits while preserving hazard sizes and horizontal timing. Create/resize derives the domain; restart reuses it. Accepted spawns and locked targets are not rerolled or retargeted. Director acceptance of pressure, recovery, and readability remains tracked by [#191](https://github.com/bongohorse/monster-girl-delivery/issues/191).

Resize also reconciles retained encounter exit samples to the new playable center bounds through the existing flight constraint rule ([#205 / PR #206](https://github.com/bongohorse/monster-girl-delivery/pull/206)). It preserves sample order/count and envelope distance, constrains outward velocity at contact, and advances no simulation time, generation/PRNG, scheduling, or hazard lifecycle. Legal samples are retained; strict transition validation still applies.
...
The Director performance HUD is read-only instrumentation split between a pure sampler and a lightweight DOM presentation. `PerformanceSampler` records wall-clock intervals between actual Foundation game-step callbacks into a fixed-capacity ring buffer in O(1) work per valid step; rolling percentiles are computed only on the capped display refresh. `DirectorPerformanceHud` derives the displayed/measured FPS from that same bounded game-step interval window rather than Phaser's RAF-level `actualFps`. NP/ZP benchmark runs force the standard 60 FPS limit, normalize the workload, monitor the allocation-free sample count, and export exactly when the first bounded window fills; percentile snapshots are not created every game step. Manual Director changes cancel an active automated benchmark. Lifecycle-paused frames, the first resume frame, preset setup, FPS-limit changes, and the first timestamp used to establish an interval baseline are rejected so suspension/setup time cannot contaminate the session. Neither measurement nor presentation feeds values back into `TimeService` or gameplay.
