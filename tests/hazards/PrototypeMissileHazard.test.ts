...
const MISSILE_LAYOUT = Object.freeze({ playerRunDistance: 630, playerScreenX: 100, viewportLeft: 0, viewportRight: 400 });
const createMissileSpawn = () => {
  const schedule = scheduleNextPattern({ catalog: [PROTOTYPE_MISSILE_PATTERN], patternStartDistance: 0, state: createRunGenerationState('m5-missile') });
  if (schedule.status !== 'accepted' || !schedule.spawns[0]) throw new Error('Expected M5 Missile fixture to produce one accepted spawn.');
  return schedule.spawns[0];
};
const getLifecycle = (state: ReturnType<typeof createTelegraphedHazardSimulationState>, spawn: ReturnType<typeof createMissileSpawn>) => {
  const lifecycle = getTelegraphedHazardLifecycle(state, spawn);
  if (!lifecycle) throw new Error('Expected Missile lifecycle.');
  return lifecycle;
};
...
describe('M5 bait-and-dodge Missile', () => {
  it('trails player movement during warning instead of snapping to the player', () => {
    ...
    let state = stepTelegraphedHazardSimulation(createTelegraphedHazardSimulationState(), [spawn], 0, { positionY: 100, runDistance: 0 });
    state = stepTelegraphedHazardSimulation(state, [spawn], 0.6, { positionY: 180, runDistance: 210 });
    expect(getLifecycle(state, spawn)).toMatchObject({ phase: 'warning', latestObservedTarget: { positionY: 148, runDistance: 210 }, lockedTarget: null });
  });

  it('commits the lagged marker, launches offscreen, and cannot retarget afterward', () => {
    const spawn = createMissileSpawn();
    let state = stepTelegraphedHazardSimulation(createTelegraphedHazardSimulationState(), [spawn], 0, { positionY: 100, runDistance: 0 });
    state = stepTelegraphedHazardSimulation(state, [spawn], 1.4, { positionY: 100, runDistance: 0 }, (delta) => ({ positionY: 100 + 50 * delta, runDistance: 350 * delta }));
    ...
    state = stepTelegraphedHazardSimulation(state, [spawn], 0.4, { positionY: 72, runDistance: 630 });
    ...
    state = stepTelegraphedHazardSimulation(state, [spawn], 0.5, { positionY: 60, runDistance: 805 });
    ...
    if (!crossing) throw new Error('Expected active Missile crossing the player lane.');
    expect(isPlayerCollidingWithHazard({ distance: 805 }, { positionY: 142, velocityY: 0 }, crossing)).toBe(true);
    expect(isPlayerCollidingWithHazard({ distance: 805 }, { positionY: 60, velocityY: 0 }, crossing)).toBe(false);
  });

  it('models launch side as data so a left-side Missile travels right', () => {
    const spawn = createMissileSpawn();
    if (!isTargetLockStrikeHazardBehavior(spawn.behavior) || !isPrototypeMissileBehavior(spawn.behavior)) throw new Error('Expected M5 Missile behavior.');
    const target = Object.freeze({ positionY: 142, runDistance: 490 });
    const leftLaunch = Object.freeze({ ...spawn, behavior: Object.freeze({ ...spawn.behavior, missile: Object.freeze({ ...spawn.behavior.missile, launchSide: 'left' as const }) }) });
  ...
  it('does not advance targeting or lifecycle on zero delta', () => {
    const spawn = createMissileSpawn();
    const state = stepTelegraphedHazardSimulation(createTelegraphedHazardSimulationState(), [spawn], 0.5, { positionY: 150, runDistance: 175 });
    const unchanged = stepTelegraphedHazardSimulation(state, [spawn], 0, { positionY: 72, runDistance: 999 });
  ...
  it('resumes warning from authoritative elapsed time without a targeting or lifecycle jump', () => {
    const spawn = createMissileSpawn();
    let state = stepTelegraphedHazardSimulation(createTelegraphedHazardSimulationState(), [spawn], 0, { positionY: 100, runDistance: 0 });
    state = stepTelegraphedHazardSimulation(state, [spawn], 0.7, { positionY: 150, runDistance: 245 });
    const beforePause = getLifecycle(state, spawn);
    const paused = stepTelegraphedHazardSimulation(state, [spawn], 0, { positionY: 40, runDistance: 999 });
    expect(paused).toBe(state);
    expect(getLifecycle(paused, spawn)).toEqual(beforePause);
    state = stepTelegraphedHazardSimulation(paused, [spawn], 0.7, beforePause.latestObservedTarget, (delta) => ({ positionY: 150 + 20 * delta, runDistance: 245 + 350 * delta }));
  ...
  it('locks to the same target and travel position across all supported frame partitions', () => {
    const spawn = createMissileSpawn();
    const results = Object.fromEntries(Object.entries(STANDARD_FRAME_SCHEDULES).map(([name, schedule]) => {
      let state = stepTelegraphedHazardSimulation(createTelegraphedHazardSimulationState(), [spawn], 0, { positionY: 100, runDistance: 0 });
      let elapsed = 0;
      let steps = 0;
      const targetTime = 2;
      while (elapsed < targetTime - 1e-12) {
        ...
        state = stepTelegraphedHazardSimulation(state, [spawn], delta, { positionY: 100 + 50 * stepStart, runDistance: 350 * stepStart }, (subDelta) => ({ positionY: 100 + 50 * (stepStart + subDelta), runDistance: 350 * (stepStart + subDelta) }));
        elapsed += delta;
        steps += 1;
      }
      const lifecycle = getLifecycle(state, spawn);
      const missile = getLethalHazardsForTelegraphedSimulation(state, [spawn], { ...MISSILE_LAYOUT, playerRunDistance: 700 })[0];
