...
const createHarness = () => {
  ...
  const startCollectiblePerformancePreset = vi.fn();
  ...
  const hud = new DirectorPerformanceHud(
    ...
    sampler,
    {
      ...
      setSimulationFrozen,
      triggerDeath,
      startCollectiblePerformancePreset,
  ...
  const collectiblePerformanceButton = playgroundControls?.children[9];
...
describe('DirectorPerformanceHud', () => {
  ...
  it('starts the collectible-heavy preset from one normalized measurement state', () => {
    const {
      autoHazardsButton,
      collectiblePerformanceButton,
      freezeButton,
    ...
    collectiblePerformanceButton.dispatch('click');

    expect(setGodModeEnabled).toHaveBeenLastCalledWith(true);
    expect(setAutoHazardsEnabled).toHaveBeenLastCalledWith(false);
    expect(setSimulationFrozen).toHaveBeenLastCalledWith(false);
    expect(setWireframesEnabled).toHaveBeenLastCalledWith(false);
    expect(startCollectiblePerformancePreset).toHaveBeenCalledOnce();
  ...
  it('removes its DOM and listeners idempotently on shutdown', () => {
    ...
    for (const element of [
      ...
      harness.deathButton,
      harness.collectiblePerformanceButton,
