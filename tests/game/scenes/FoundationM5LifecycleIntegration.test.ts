...
import type { PrototypeRunResultSnapshot } from '../../../src/systems/PrototypeRunResult';
...
const PLAYER_Y = 195;
const GRAZE_HITBOX = Object.freeze({ left: 800, right: 820, top: 160, bottom: 168 });
const LETHAL_HITBOX = Object.freeze({ left: 1_100, right: 1_130, top: 180, bottom: 210 });
...
interface LifecycleHarness {
  ...
  readonly generatedHazardPresentation: { destroy: ReturnType<typeof vi.fn>; sync: ReturnType<typeof vi.fn> };
  readonly initialStream: Readonly<GeneratedHazardStreamState>;
  ...
  readonly scrollingWorldPresentation: {
    destroy: ReturnType<typeof vi.fn>;
    render: ReturnType<typeof vi.fn>;
  };
  readonly services: ReturnType<typeof createAppServices>;
}

interface LifecycleEvidence {
  readonly deathDistance: number;
  readonly deathElapsedSeconds: number;
...
const createStaticSpawn = (
  ...
  hitbox: Readonly<typeof GRAZE_HITBOX>,
...
const createHarness = (): LifecycleHarness => {
  ...
  const customSpawns = Object.freeze([
...
): { readonly elapsedSeconds: number; readonly steps: number } => {
...
describe('Foundation integrated M5 arcade lifecycle', () => {
  it('preserves the same Graze/death/retry semantics across standard frame partitions', () => {
    ...
    for (const [name, schedule] of SCHEDULE_ENTRIES) {
      ...
      const runEvidence = runUntilDead(foundation, schedule);
      ...
      const stalePressDelta = schedule.getNextDelta(0, 0);
      foundation.update(0, stalePressDelta * 1_000);
      ...
      expect(getTelegraphedState(foundation)).not.toBe(deadTelegraphs);
      expect(services.input.isThrustHeld()).toBe(false);
      expect(services.input.consumePrimaryActionPress()).toBe(false);
      expect(finalResult).toEqual({ ...finalResult });
      ...
      evidence.push({
        ...
        deathElapsedSeconds: runEvidence.elapsedSeconds,
