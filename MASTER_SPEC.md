...
The recent-history variety policy retains the last four accepted variety-family identities by default after M5 Director playtesting showed that the earlier two-family window repeated recognizable ideas too quickly. It consumes no random values, never records rejected candidates, and suppresses recent family repetition while fresh alternatives remain in the already-eligible catalog. Recent candidates stay available as an authored-order deterministic fallback when content is scarce or fresh candidates fail hard fairness, without restoring anything excluded by pacing or difficulty. Explicit **PROTOTYPE** policy data may allow deliberate family recurrence. History is immutable, serializable, bounded independently of run length, and unaffected by viewport dimensions; live orchestration with the other encounter policies landed in #120.
