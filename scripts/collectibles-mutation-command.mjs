...
try {
  ...
  evidence = {
    id, exitCode: child.status, completed: completed.length,
    kind: !complete || report.numRuntimeErrorTestSuites > 0 || child.error
      ? 'RuntimeError'
      : child.status === 0 && report.success && failed.length === 0 ? 'Passed'
      : failed.length > 0 ? 'TestFailure' : 'RuntimeError',
    failures: failed.map((test) => ({
      name: test.fullName, messages: test.failureMessages,
    })),
  };
} catch (error) {
