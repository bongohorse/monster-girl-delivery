...
export class DirectorPerformanceHud {
  ...
  private preparePerformancePreset(autoHazardsEnabled: boolean): void {
    if (!this.godModeEnabled) {
      this.godModeEnabled = true;
      this.setToggleState(this.godModeButton, true);
      this.controls?.setGodModeEnabled?.(true);
    }
    if (this.autoHazardsEnabled !== autoHazardsEnabled) {
      this.autoHazardsEnabled = autoHazardsEnabled;
      this.setToggleState(this.autoHazardsButton, autoHazardsEnabled);
      this.controls?.setAutoHazardsEnabled?.(autoHazardsEnabled);
    }
